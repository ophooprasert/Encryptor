...
@measure_time('Thread_time')
def decrypt_thread(read_file, encoding, _queue):
    global file_queue
    found_entry = False
    found_err = False
    new_file = ''
    ...
    try:
        with open(encrypted_file, 'r') as refile:
           ...
           # print(len(read_file))
            ...
            if len(read_file) > 8000:
               # print len(read_file)%8
                equal_div = len(read_file)/num_of_threads
                part_length = equal_div - equal_div%8
                #print part_length
                nstart = 0
                for div in range(num_of_threads+1):
                    div_files.append(read_file[nstart:nstart + part_length])
                    nstart += part_length
            count = 0
            total = 0
            for n in div_files:
            ...
            d_threads = []
            count1 = 0
            for _part in div_files:
                dt_1 = threading.Thread(target=decrypt_thread, args=(_part, encoding, count1))
                dt_1.start()
                d_threads.append(dt_1)
                count1 += 1
            for t in d_threads:
                t.join()
            print_dict2(file_queue)
            for i in range(count1):
                new_file += file_queue[i]
            '''
            for ind in range(len(read_file)):
                if ind%8 == 0:
                    if read_file[ind-8:ind] in encoding:
